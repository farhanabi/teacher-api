...
describe('POST /api/register', () => {
  ...
  afterEach(async () => {
    for (const teacherEmail of testResources.teacherEmails) {
      const teacherRecord = await db.select().from(teachers).where(eq(teachers.email, teacherEmail));
      if (teacherRecord.length > 0) {
        const teacherId = teacherRecord[0]!.id;
        await db.delete(registrations).where(eq(registrations.teacherId, teacherId));
        await db.delete(teachers).where(eq(teachers.id, teacherId));
      }
    }

    for (const studentEmail of testResources.studentEmails) {
      const studentRecord = await db.select().from(students).where(eq(students.email, studentEmail));
  ...
  it('should register students to a teacher', async () => {
    ...
    const teacher = await db.select().from(teachers).where(eq(teachers.email, testTeacherEmail));
    ...
    for (const studentEmail of testStudentEmails) {
      ...
      const registrationResults = await db
        ...
        .where(
          and(
            eq(registrations.teacherId, teacher[0]!.id),
            eq(registrations.studentId, student[0]!.id)
          )
        );
