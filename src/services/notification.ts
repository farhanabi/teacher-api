...
export const getNotificationRecipients = async (teacherEmail: string, notification: string) => {
  const mentionedStudentEmails = extractMentionedEmails(notification);

  const teacher = await db.select().from(teachers).where(eq(teachers.email, teacherEmail)).limit(1);
  ...
  const allRecipients = [...new Set([...registeredStudentEmails, ...mentionedNonSuspendedStudents])];
